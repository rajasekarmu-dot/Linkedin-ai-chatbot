...
import { inMemoryDB, ContactRecord, FollowupRecord } from '../../lib/supabase';
...
export async function handleCron(req: Request, res: Response) {
  try {
    ...
    const staleDigest = inMemoryDB.contacts
      .filter((c: ContactRecord) => new Date(c.last_contacted_at) <= fourWeeksAgo)
      .map((c: ContactRecord) => ({
        contactId: c.id,
        name: c.name,
        phone: c.phone,
    ...
    const reEngagementTargets = inMemoryDB.contacts.filter(
      (c: ContactRecord) =>
  new Date(c.last_contacted_at) <= thirtyDaysAgo && c.status === 'active'
    );
